...
async function seedRuntime() {
  ...
  try {
    ...
    const [anita, felix, employeesForSeed] = await Promise.all([
      prisma.user.findUniqueOrThrow({ where: { email: 'anita@sankofa.local' } }),
      prisma.user.findUniqueOrThrow({ where: { email: 'felix@sankofa.local' } }),
      prisma.employee.findMany({ orderBy: { employeeCode: 'asc' } }),
    ]);

    const employeeByCode = new Map(employeesForSeed.map((employee) => [employee.employeeCode, employee]));
    ...
    const entryByCode = new Map(reviewTwoEntries.map((entry) => [entry.employee.employeeCode, entry]));
